...
def establish_connection():
    conn = db.connect_db('live_db', 'root', 'admin@123', 'localhost')
    return conn

# Plot the number of matches played per year of all the years in IPL.


def num_of_match_each_year(match_table_name):
...
def stacked_bar_chart(match_table_name):
...
def match_2016_extra_run(match_table_name, delivery_table_name):
    conn_obj = establish_connection()
    cursor = conn_obj.cursor()
...
def match_2015_eco_bowler(match_table_name, delivery_table_name):
...
def match_summary_over_years(match_table_name, season):
...
def choose_option():
    while True:
        ...
        if isinstance(selection_var, int) and selection_var in [1, 2, 3, 4, 5, 6]:
            if selection_var == 1:
                lst_of_season, num_of_matches_played = num_of_match_each_year('matches')
                ...
                data_dict, lst_of_winning_team = stacked_bar_chart('matches')
                ...
                extra_run_per_team = match_2016_extra_run('matches', 'deliveries')
                ...
                bowler_eco_rate = match_2015_eco_bowler('matches', 'deliveries')
                ...
                player_of_match = match_summary_over_years('matches', str(season))
